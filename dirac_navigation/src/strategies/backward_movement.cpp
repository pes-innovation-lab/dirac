...
void BackwardMovement::execute(const MovementContext& context)
{
    RCLCPP_INFO(context.node->get_logger(), 
               "Agent %d: Executing backward movement (%.2f units at %.2f m/s)",
               context.agent_id, context.parameters.move_distance, context.parameters.linear_speed);
    
    // Create and publish backward movement command
    geometry_msgs::msg::Twist twist_msg;
    twist_msg.linear.x = -context.parameters.linear_speed;  // Negative for backward
