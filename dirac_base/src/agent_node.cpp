...
int main(int argc, char ** argv)
{
  ...
  node->declare_parameter<std::string>("agent_id", "agent_1");
  node->declare_parameter<std::string>("zone_id", "1");
  ...
  node->declare_parameter<std::string>("z_leader", "");

  // Get parameters
  std::string agent_id = node->get_parameter("agent_id").as_string();
  std::string zone_id = node->get_parameter("zone_id").as_string();
