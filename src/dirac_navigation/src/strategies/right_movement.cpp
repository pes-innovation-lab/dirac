...
void RightMovement::execute(const MovementContext& context)
{
    RCLCPP_INFO(context.node->get_logger(), 
               "Agent %d: Executing right movement (turn-move-turn sequence)",
               context.agent_id);
    ...
    executeTurn(context, -context.parameters.angular_speed, [this, context]() {
        ...
        executeForward(context, [this, context]() {
            // Step 3: Turn left 90 degrees to restore original orientation
            executeTurn(context, context.parameters.angular_speed, [this, context]() {
                stopAgent(context);
            });
        });
...
void RightMovement::executeTurn(const MovementContext& context, 
...
void RightMovement::executeForward(const MovementContext& context, 
                                  std::function<void()> callback)
{
